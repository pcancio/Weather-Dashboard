...
// get the weather today and the date
    ...
    fetch(apiLongLat).then(function(response) {
        if (response.ok) {
            response.json().then(function(data) {
                ...
                currentWeatherCardImg.setAttribute(
                    "src",
                ...
                currentWeatherCardTemp.textContent =
                    "Current Temperature (f) = " + data.current.temp;
                ...
                currentWeatherCardWind.textContent =
                    "Current Wind Speed (mph) = " + data.current.wind_speed;
                ...
                for (var i = 0; i < 5; i++) {
                    ...
                    dailyWeatherCardImg.setAttribute(
                        "src",
                        "http://openweathermap.org/img/wn/" +
                    ...
                    dailyWeatherCardTemp.textContent =
                        "Anticipated Temperature (f) = " + data.daily[i].temp.day;
                    var dailyWeatherHumidity = document.createElement("li");
                    dailyWeatherHumidity.textContent =
                        "Anticipated Humidity = " + data.daily[i].humidity;
                    var dailyWeatherCardWind = document.createElement("li");
                    dailyWeatherCardWind.textContent =
                        "Anticipated Wind Speed (mph) = " + data.daily[i].wind_speed;
                    var dailyWeatherCardUVI = document.createElement("li");
                    dailyWeatherCardUVI.textContent =
                        "Anticipated UVI = " + data.daily[i].uvi;
